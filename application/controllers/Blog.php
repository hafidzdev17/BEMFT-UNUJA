...
class Blog extends CI_Controller
{
	...
	function detail($slugs)
	{
		...
		if ($query->num_rows() > 0) {
			...
			$this->load->view('layout/header2');
			$this->load->view('depan/berita_detail', $x);
			$this->load->view('layout/footer');
		} else {
			redirect('artikel');
		}
	...
	function kategori()
	{
		...
		if ($query->num_rows() > 0) {
			...
			$x['category'] = $this->db->get('tbl_kategori');
			$x['populer'] = $this->db->query("SELECT * FROM tbl_tulisan ORDER BY tulisan_views DESC LIMIT 5");
			$x['judul'] = 'BEM FT | Daftar Kategori';
	...
	function search()
	{
		...
		if ($query->num_rows() > 0) {
			...
			$x['category'] = $this->db->get('tbl_kategori');
			$x['populer'] = $this->db->query("SELECT * FROM tbl_tulisan ORDER BY tulisan_views DESC LIMIT 5");
			$x['judul'] = 'BEM FT | Daftar Pencarian Berita';
	...
	function komentar()
	{
		...
		if (empty($nama) || empty($email)) {
			...
			$this->session->set_flashdata('msg', '<div class="alert alert-info">Komentar Anda akan tampil setelah <b> disetujui</b>.</div>');
			redirect('artikel/' . $slug);
		}
	}
}
